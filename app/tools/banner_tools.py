...
async def create_banner(url: str, position: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        response = await make_request(client, "post", f"{BE_BASE_URL}/api/banners", json={"url": url, "position": position})
        response.raise_for_status()
        return response.json()
...
async def update_banner(banner_id: str, url: str, position: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        response = await make_request(client, "put", f"{BE_BASE_URL}/api/banners/{banner_id}", json={"url": url, "position": position})
        response.raise_for_status()
        return response.json()
...
async def delete_banner(banner_id: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        response = await make_request(client, "delete", f"{BE_BASE_URL}/api/banners/{banner_id}")
        response.raise_for_status()
