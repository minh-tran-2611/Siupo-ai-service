...
async def get_all_notifications_admin() -> dict:
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        response = await make_request(client, "get", f"{BE_BASE_URL}/api/notifications/admin")
...
async def create_notification(title: str, content: str, user_id: int = None, send_to_all: bool = False) -> dict:
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        response = await make_request(client, "post", f"{BE_BASE_URL}/api/notifications/admin", json=data)
        response.raise_for_status()
        return response.json()
...
async def get_my_notifications() -> dict:
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        response = await make_request(client, "get", f"{BE_BASE_URL}/api/notifications/customer")
        response.raise_for_status()
        return response.json()
